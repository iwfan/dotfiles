...
local function generate_quota()
    local random_idx = math.random(#predefined_quotes)
    local quota = predefined_quotes[random_idx]
    local quota_content = quota[1]
    local insert_pos = 1
    local splitted_chunk_count = 60
    local isOutOfCapacity = string.len(quota_content) > splitted_chunk_count

    if isOutOfCapacity then
        table.remove(quota, 1)
    end

    while string.len(quota_content) > splitted_chunk_count do
        local chunk = string.sub(quota_content, 1, splitted_chunk_count)
        quota_content = string.sub(quota_content, splitted_chunk_count + 1, -1)
        table.insert(quota, insert_pos, chunk)
        insert_pos = insert_pos + 1
    end

    if isOutOfCapacity and string.len(quota_content) > 0 then
        table.insert(quota, insert_pos, quota_content)
    end

    local blank_count = splitted_chunk_count - string.len(quota[#quota])
    quota[#quota] = string.rep(" ", blank_count) .. quota[#quota]
